...
| ENOE microdatos | [21/21 rutas idénticas con Neon como puente](enoe-microdatos-datos.md); desde el 2026-09-20 se sirven desde particiones Parquet en R2 con **paginación por cursor** (cambio de contrato documentado: `cursor`/`next_cursor` en lugar de `page`; orden entidad primero sin filtro de entidad). Verificado contra el legacy: conteos, páginas fila a fila, recorrido completo de una partición y cruce de entidades idénticos; FALLOS: 0. | (incluidas en ENOE) | 12,768 particiones = 101,512,667 filas = origen; índice en D1 verificado. |
...
- ENOE microdatos: ya no dependen de Neon ni de Hyperdrive; se leen de particiones Parquet en R2 desde el worker. El contrato cambió de `page` a cursor (decisión del CEO 2026-09-20).
