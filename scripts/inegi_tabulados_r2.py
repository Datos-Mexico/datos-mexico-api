...
def procesar(x):
    ...
    try:
        ...
        clave = f'inegi/tabulados/{prog}/{ed}/{nombre.replace(" ", "_")}'; g.subir(clave, f, tipo)
...
def main():
    ...
    for x in todos: grupos[(x['programa'], x['anio'], pathlib.Path(x['path']).name)].add(x['id'])
    g.COLISIONES_TAB.update(k for k, v in grupos.items() if len(v) > 1)
