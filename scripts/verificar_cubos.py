...
# Exclusión 8 cerrada (docs/METODOLOGIAS-INEGI.md): prevalencia delictiva = 6200002197, ENVIPE 2020 = levantamiento de marzo, ENSANUT = INSP
d, _, _ = get("/api/v1/cubos/envipe-prevalencia/datos?medidas=tasa,victimas,personas&columnas=anio&f.anio=2025")
check(d and d["n"] == 1 and abs(d["filas"][0]["tasa"] - 23472.5354647741) < 0.01 and d["filas"][0]["personas"] == 98144570, f"ENVIPE prevalencia 2025: {d and d['filas'][0]['tasa']:.4f} por 100 mil = indicador 6200002197 (23,472.5)")
d, _, _ = get("/api/v1/cubos/envipe-percepcion/datos?medidas=pct_inseguro,personas&columnas=anio&f.anio=2020&f.ambito=colonia")
check(d and d["n"] == 1 and abs(d["filas"][0]["pct_inseguro"] - 48.7372827584775) < 0.005 and d["filas"][0]["personas"] == 35919641, f"ENVIPE 2020 (solo marzo): {d and d['filas'][0]['pct_inseguro']:.4f} % inseguro en la colonia = 48.74 publicado; {d and d['filas'][0]['personas']:,} personas = cuadro 5.4")
d, _, _ = get("/api/v1/cubos/ensanut-imc/datos?medidas=pct_sobrepeso,pct_obesidad,personas,n&columnas=edicion")
check(d and d["n"] == 1 and abs(d["filas"][0]["pct_sobrepeso"] - 39.11) < 0.01 and abs(d["filas"][0]["pct_obesidad"] - 36.07) < 0.01 and d["filas"][0]["n"] == 16579 and d["filas"][0]["personas"] == 77708132, f"ENSANUT 2018-19: sobrepeso {d and d['filas'][0]['pct_sobrepeso']:.2f} %, obesidad {d and d['filas'][0]['pct_obesidad']:.2f} %, n = {d and d['filas'][0]['n']:,} = INSP (39.1/36.1, 16,579)")
...
check(d and d["total"] == 423302, "ENOE microdatos 2025T1 rehecho desde el CSV oficial: 423,302 (el legado tenía 409,796)")
...
# DENUE histórico (20 ediciones; scripts/denue_historico.py)
e, _, _ = get("/api/v1/denue/ediciones")
eds = {x["edicion"]: x for x in e["items"]}
check(e["n"] == 25 and eds["2026-05"]["unidades"] == 6138075 and eds["2026-05"]["verificado"] is True and eds["2015"]["unidades"] == 4926061 and eds["2015"]["verificado"] is True, f"DENUE ediciones: {e['n']}; 2026-05 {eds.get('2026-05', {}).get('unidades')}, 2015 {eds.get('2015', {}).get('unidades')}")
check(all((x["publicado"] is None) == (x["verificado"] is None) and (x["publicado"] is None or x["diferencia"] == x["unidades"] - x["publicado"] and x["verificado"] == (x["diferencia"] == 0)) for x in e["items"]), "DENUE ediciones: diferencia y verificado coherentes con la cifra publicada")
d, _, _ = get("/api/v1/cubos/denue-historico/datos?medidas=unidades&columnas=edicion&orden=edicion&sentido=asc")
check(d["n"] >= 9 and all(f["unidades"] == eds[f["edicion_id"]]["unidades"] and eds[f["edicion_id"]]["verificado"] is not False for f in d["filas"]), f"DENUE histórico: {d['n']} ediciones en el cubo, todas con el total de sus archivos y ninguna no verificada")
check([f["edicion_id"] for f in d["filas"]] == sorted(f["edicion_id"] for f in d["filas"]) and d["filas"][-1]["edicion_id"] == "2026-05", "DENUE histórico: ediciones en orden cronológico hasta 2026-05")
get("/api/v1/cubos/denue-historico-municipal/datos?medidas=unidades&columnas=entidad", 422)
d, _, _ = get("/api/v1/cubos/denue-historico-municipal/datos?medidas=unidades&columnas=entidad&f.edicion=2026-05")
check(d["n"] == 32 and sum(f["unidades"] for f in d["filas"]) == tot_denue, f"DENUE histórico municipal 2026-05: 32 entidades, {sum(f['unidades'] for f in d['filas']):,} = DENUE por entidad {tot_denue:,} (archivos por sector = archivos por entidad)")
d2, _, _ = get("/api/v1/cubos/denue-unidades/datos?medidas=unidades&columnas=municipio&f.entidad=09")
d3, _, _ = get("/api/v1/cubos/denue-historico-municipal/datos?medidas=unidades&columnas=municipio&f.edicion=2026-05&f.entidad=09")
check({f["municipio_id"]: f["unidades"] for f in d2["filas"]} == {f["municipio_id"]: f["unidades"] for f in d3["filas"]}, f"DENUE 2026-05: los {d3['n']} municipios de la CDMX coinciden unidad por unidad entre la edición vigente y el histórico")
s, _, _ = get("/api/v1/denue/historico?codigo_act=46&cve_ent=09")
check(s["n"] == 25 and all(x["unidades"] > 0 for x in s["items"]) and s["items"][-1]["edicion"] == "2026-05", f"DENUE serie comercio al por menor CDMX: {s['n']} ediciones")
get("/api/v1/denue/historico/descarga/1999", 404)
# Clasificadores del INEGI (scripts/clasificadores_d1.py) y Marco Geoestadístico 2025 (scripts/mg_2025.py)
cl, _, _ = get("/api/v1/clasificadores")
check(cl["n"] == 9 and all(all(v["obtenido"] == v["publicado"] for v in x["verificacion"] if v["publicado"] is not None) for x in cl["items"]), "clasificadores: 9 catálogos, conteos por nivel = publicados por el INEGI")
sc, _, _ = get("/api/v1/clasificadores/scian?nivel=clase&limit=2000")
check(sc["total"] == 1086 and get("/api/v1/clasificadores/scian?nivel=clase&version=2018&limit=1")[0]["total"] == 1084 and get("/api/v1/clasificadores/scian?nivel=clase&version=2013&limit=1")[0]["total"] == 1059, "SCIAN: 1,086 clases (2023), 1,084 (2018), 1,059 (2013)")
f, _, _ = get("/api/v1/clasificadores/scian/722511")
check([a["codigo"] for a in f["ancestros"]] == ["72", "722", "7225", "72251"] and f["productos"], "SCIAN 722511: ancestros 72 › 722 › 7225 › 72251 y productos")
si, _, _ = get("/api/v1/clasificadores/sinco?nivel=grupo_unitario&limit=1"); check(si["total"] == 490, f"SINCO 2019: {si['total']} grupos unitarios")
lo, _, _ = get("/api/v1/geo/localidades?limit=1"); mu, _, _ = get("/api/v1/geo/municipios"); en, _, _ = get("/api/v1/geo/entidades")
check(lo["total"] == 296633 and mu["n"] == 2478 and en["n"] == 32, f"AGEEML: {lo['total']:,} localidades, {mu['n']:,} municipios, {en['n']} entidades")
get("/api/v1/geo/localidades?cve_mun=015", 422); get("/api/v1/clasificadores/scian?version=1999", 422)
mg, _, _ = get("/api/v1/inegi/mg")
check(mg["edicion"] == "2025" and all(v["obtenido"] == v["publicado"] for v in mg["verificacion"].values()) and len(mg["capas"]) == 16, f"MG 2025: 16 capas, verificación {mg['verificacion']}")
