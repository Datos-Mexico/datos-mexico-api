...
export const BIE_SERIES: Cubo = {
  ...
  dimensiones: [
    ...
    { clave: "desglose", titulo: "Desglose geográfico", id: "a.desglose", tipo: "categorica", descripcion: "Nacional (México), Estatal, Municipal o País (otros países con los que el INEGI compara). Los niveles contienen a la misma economía: toda consulta filtra uno o agrupa por él (o por área).", particion: { implicita_en: ["area"] }, miembros: { desde: "(SELECT DISTINCT desglose FROM areas) x", id: "x.desglose", nombre: "x.desglose", orden: "CASE x.desglose WHEN 'Nacional' THEN 1 WHEN 'Estatal' THEN 2 WHEN 'Municipal' THEN 3 ELSE 4 END" } },
    { clave: "area", titulo: "Área geográfica", id: "o.area", nombre: "a.nombre", tipo: "geografica", geo: "entidad", orden: "1", descripcion: "00 = México; 01-32 = entidades; 5 dígitos = municipios; 4 dígitos = otros países.", miembros: { desde: "areas a", id: "a.clave", nombre: "a.nombre", orden: "1" } },
