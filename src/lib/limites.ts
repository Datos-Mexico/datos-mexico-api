...
const CUPOS: Record<string, number> = {
  ...
  "/api/v1/export/csv": 5,
  "/api/v1/admin/refresh-materialized-views": 5, "/api/v1/admin/espejo": 600, "/api/v1/demo/estudiantes": 60, "/api/v1/demo/resumen": 60, "/api/v1/admin/demo/estudiantes": 20, "/api/v1/admin/demo/reset": 20,
...
function cupo(ruta: string): number {
  ...
  if (/^\/api\/v1\/demo\/estudiantes\/[^/]+\/toggle-bono$/.test(ruta)) return 30;
  if (/^\/api\/v1\/demo\/estudiantes\/[^/]+$/.test(ruta)) return 120;
  if (/^\/api\/v1\/admin\/demo\/estudiantes\/[^/]+$/.test(ruta)) return 20;
  ...
  if (/^\/api\/v1\/bie\/series\/[^/]+\/observaciones$/.test(ruta)) return 30;
  ...
  if (/^\/api\/v1\/cubos\/[^/]+\/(datos|miembros)$/.test(ruta)) return 30;
  return 60;
}
...
export const limitarPeticiones: MiddlewareHandler<{ Bindings: Env }> = async (c, next) => {
  ...
  const n = cupo(ruta);
  const limitador = n === 5 ? c.env.RL_5 : n === 10 ? c.env.RL_10 : n === 15 ? c.env.RL_15 : n === 20 ? c.env.RL_20 : n === 30 ? c.env.RL_30 : n === 120 ? c.env.RL_120 : n === 600 ? c.env.RL_600 : c.env.RL_60;
  if (limitador) {
